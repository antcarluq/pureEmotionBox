 ...
 */
if ( ! function_exists( 'zurbox_topbar_r' ) ) {
	function zurbox_topbar_r() {
		wp_nav_menu( array(
			'container'      => false,
			...
			'depth'          => 4,
			'fallback_cb'    => false,
			'walker'         => new Zurbox_Top_Bar_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'zurbox_topbar_l' ) ) {
	function zurbox_topbar_l() {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'dropdown menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s show-for-medium" data-dropdown-menu>%3$s</ul>',
 ...
 */
if ( ! function_exists( 'zurbox_mobile_nav' ) ) {
	function zurbox_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false, // Remove nav container
			'menu'           => esc_html( 'mobile-nav' ),
			'menu_class'     => 'vertical menu',
			'theme_location' => 'mobile-nav',
			'items_wrap'     => '<ul id="%1$s" class="%2$s show-for-small-only" data-accordion-menu>%3$s</ul>',
			'fallback_cb'    => false,
			'walker'         => new Zurbox_Mobile_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'zurbox_add_menuclass' ) ) {
	function zurbox_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','zurbox_add_menuclass' );
}
 ...
 */
if ( ! function_exists( 'zurbox_breadcrumb' ) ) {
	function zurbox_breadcrumb( $showhome = true, $separatorclass = false ) {

		// Settings
		$separator  = '&gt;';
		$id         = 'breadcrumbs';
		$class      = 'breadcrumbs';
		$home_title = esc_html__( 'Home', 'zurbox-lite' );
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && isset($category[0]) ) {
			...
			} else if ( is_category() && isset($category[0]) ) {
				...
				echo '<li class="item-current"><strong class="bread-current">' . $post_type->label . '</strong></li>';

			} else if ( is_single() && 'post' != get_post_type() ) {
				// Post Type single
				$post_type = get_post_type_object( get_post_type() );
				echo '<li class="item-type"><a class="bread-cat" href="' . get_post_type_archive_link( get_post_type() ) . '">' . $post_type->label . '</a>';
				if ( $separatorclass ) {
					echo '<li class="separator"> ' . $separator . ' </li>';
				}
				echo '<li class="item-current"><strong class="bread-current">' . get_the_title() . '</strong></li>';

			} else if ( is_tax() && 'post' != get_post_type() ) {
				$post_type = get_post_type_object( get_post_type() );
				echo '<li class="item-type"><a class="bread-cat" href="' . get_post_type_archive_link( get_post_type() ) . '">' . $post_type->label . '</a>';
				...
				// Display author name
				echo '<li class="current item-current-' . $userdata->user_nicename . '">' . esc_html__( 'Author:', 'zurbox-lite' ) . $userdata->display_name . '</li>';

			} else if ( get_query_var('paged') ) {

				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . esc_html__( 'Page', 'zurbox-lite' ) . ' ' . get_query_var('paged') . '</li>';

			} else if ( is_search() ) {

				// Search results page
				echo '<li class="current item-current-' . get_search_query() . '">' . esc_html__( 'Search Reasults for: ', 'zurbox-lite' ) . get_search_query() . '</li>';

			} elseif ( is_404() ) {

				// 404 page
				echo '<li>' . esc_html__( 'Error 404', 'zurbox-lite' ) . '</li>';
			}
		} else {
			if ( $showhome ) {
